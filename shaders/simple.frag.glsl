...
layout(std140, set = 3, binding = 0) buffer SSBO
{
...
//
//     vec3 n = (material.normalTextureSet > -1) ? getNormal(material) : normalize(vec3(1.0, 1.0, 1.0));
//     vec3 v = normalize(inCamPos - inWorldPos); // Vector from surface point to camera
...
//         switch (index) {
//             case 1:
//             // outColor.rgba = material.baseColorTextureSet > -1 ? texture(colorMap, material.baseColorTextureSet == 0 ? inUV0 : inUV1) : normalize(inNormal);
...
//             outColor.rgb = (material.normalTextureSet > -1) ? texture(normalMap, material.normalTextureSet == 0 ? inUV0 : inUV1).rgb : normalize(vec3(1.0, 1.0, 1.0));
//             break;
//             case 3:
...
void main()
{
    ...
    outColor = material.baseColorFactor * texture(colorMap, inUV0);
}
